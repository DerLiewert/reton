  ...
  whyItems.forEach((whyItem) => {
    ...
    function onClick() {
      ...
      if (whyItem.classList.contains('_active')) {
        ...
        whyItem
          .querySelector('.why-item__icon')
          .setAttribute('aria-label', 'Показати детальну інформацію');
      } else {
        ...
        whyItem
          .querySelector('.why-item__icon')
          .setAttribute('aria-label', 'Приховати детальну інформацію');
      }
...
document.querySelectorAll('[data-tabs]').forEach((tabBlock) => {
  ...
  if (activeTab) {
    ...
    tabBodyItems[0].style.removeProperty('display');
  }
...
awardsTabBlocks.forEach((awardsTabs) => {
  ...
  if (isMobile.any()) {
    // для мобильных
    awardsNavWrapper.addEventListener('touchstart', onAwardsNavTouchStart, { passive: true });
    awardsNavWrapper.addEventListener('touchend', onAwardsNavTouchEnd);
    awardsContent.addEventListener('touchstart', onAwardsContentTouchStart, { passive: true });
  ...
  // Взаимодействие с awards-tabs__navigation-block (табами)
  function onAwardsNavTouchStart(e) {
    setStartPoints(e);

    const moveEvent = e.type === 'touchstart' ? 'touchmove' : 'mousemove';
    if (getWindowWidth() >= 768) {
      awardsNavWrapper.addEventListener(moveEvent, onAwardsNavVerticalMove, { passive: false });
    } else {
      awardsNavWrapper.addEventListener(moveEvent, onAwardsNavHorisontalMove, { passive: true });
    }
  }
  function onAwardsNavTouchEnd(e) {
    ...
    if (getWindowWidth() >= 768) {
      awardsNavWrapper.removeEventListener(moveEvent, onAwardsNavVerticalMove);
    } else {
      awardsNavWrapper.removeEventListener(moveEvent, onAwardsNavHorisontalMove);
    }
  }
  ...
  // Взаимодействие с awards-tabs__content (телом/контентом таба)
  function onAwardsContentTouchStart(e) {
    setStartPoints(e);
    const moveEvent = e.type === 'touchstart' ? 'touchmove' : 'mousemove';
    awardsContent.addEventListener(moveEvent, onAwardsContentMove, { passive: true });
  }
  ...
  // Общие фунркции для взаимодействия с awards-tabs__navigation-block / awards-tabs__content
  ...
  // Удаление стилей у предыдущего активного таба и добавление их активному
  ...
  // Disable кнопки для переключения табов prev/next если активный первый/последний таб
  function disabledButton() {
    let activeTab = awardsTabs.querySelector('._tab-active');
    let activeTabIndex = +activeTab.getAttribute('data-index');

    if (activeTabIndex === tabTitles.length - 1) buttonNext.disabled = true;
    else if (buttonNext.disabled) buttonNext.disabled = false;

    if (activeTabIndex === 0) buttonPrev.disabled = true;
    else if (buttonPrev.disabled) buttonPrev.disabled = false;
  }
});
