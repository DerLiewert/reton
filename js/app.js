...
*/
            (function(window) {
                ...
                if (!document.querySelectorAll) document.querySelectorAll = function(selectors) {
                ...
                if (!document.querySelector) document.querySelector = function(selectors) {
                    var elements = document.querySelectorAll(selectors);
                    return elements.length ? elements[0] : null;
                };
                if (!document.getElementsByClassName) document.getElementsByClassName = function(classNames) {
                ...
                if (!Array.prototype.forEach) Array.prototype.forEach = function(fun) {
                    ...
                    var i, thisp = arguments[1];
                    for (i = 0; i < len; i++) if (i in t) fun.call(thisp, t[i], i, t);
                };
                (function(global) {
                    var B64_ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";
                    global.atob = global.atob || function(input) {
                        input = String(input);
                        var n, position = 0, output = [], buffer = 0, bits = 0;
                    ...
                    global.btoa = global.btoa || function(input) {
                        input = String(input);
                        var o1, o2, o3, e1, e2, e3, e4, position = 0, out = [];
                ...
                if (!window.requestAnimationFrame) if (window.webkitRequestAnimationFrame && window.webkitCancelAnimationFrame) (function(global) {
                ...
                })(window); else if (window.mozRequestAnimationFrame && window.mozCancelAnimationFrame) (function(global) {
                    global.requestAnimationFrame = function(callback) {
                ...
                return function(modules) {
                    ...
                    (function(global) {
                        ...
                        var Holder = {
                            ...
                            setResizeUpdate: function(el, value) {
                                if (el.holderData) {
                                    ...
                                    if (el.holderData.resizeUpdate) updateResizableElements(el);
                                }
                            },
                            run: function(userOptions) {
                                ...
                                if (App.setup.renderers.join(",").indexOf(engineSettings.renderer) === -1) engineSettings.renderer = App.setup.supportsSVG ? "svg" : App.setup.supportsCanvas ? "canvas" : "html";
                                ...
                                bgnodes.forEach((function(bgNode) {
                                    ...
                                    if (holderStringIndex === 0) holderURL = rawURL; else if (holderStringIndex === 1 && rawURL[0] === "?") holderURL = rawURL.slice(1); else {
                                        var fragment = rawURL.substr(holderStringIndex).match(/([^"]*)"?\)/);
                                        if (fragment !== null) holderURL = fragment[1]; else if (rawURL.indexOf("url(") === 0) throw "Holder: unable to parse background URL: " + rawURL;
                                    }
                                    if (holderURL) {
                                        var holderFlags = parseURL(holderURL, options);
                                        if (holderFlags) prepareDOMElement({
                                            mode: "background",
                                            el: bgNode,
                                            flags: holderFlags,
                                            engineSettings
                                ...
                                objects.forEach((function(object) {
                                    ...
                                    if (objectHasSrcURL) prepareImageElement(options, engineSettings, objectAttr.data, object); else if (objectHasDataSrcURL) prepareImageElement(options, engineSettings, objectAttr.dataSrc, object);
                                }));
                                images.forEach((function(image) {
                                    ...
                                    if (imageHasSrc) {
                                        if (imageAttr.src.indexOf(options.domain) === 0) prepareImageElement(options, engineSettings, imageAttr.src, image); else if (imageHasDataSrcURL) if (imageRendered) prepareImageElement(options, engineSettings, imageAttr.dataSrc, image); else (function(src, options, engineSettings, dataSrc, image) {
                                            utils.imageExists(src, (function(exists) {
                                                if (!exists) prepareImageElement(options, engineSettings, dataSrc, image);
                                            }));
                                        })(imageAttr.src, options, engineSettings, imageAttr.dataSrc, image);
                                    } else if (imageHasDataSrcURL) prepareImageElement(options, engineSettings, imageAttr.dataSrc, image);
                                }));
                                return this;
                            }
                        };
                        ...
                        function parseURL(url, instanceOptions) {
                            ...
                            var parts = [ url ];
                            if (firstQuestionMark !== -1) parts = [ url.slice(0, firstQuestionMark), url.slice(firstQuestionMark + 1) ];
                            ...
                            if (parts.length === 2) {
                                ...
                                if (options.bg) holder.theme.bg = utils.parseColor(options.bg);
                                if (options.fg) holder.theme.fg = utils.parseColor(options.fg);
                                if (options.bg && !options.fg) holder.autoFg = true;
                                if (options.theme && Object.prototype.hasOwnProperty.call(holder.instanceOptions.themes, options.theme)) holder.theme = extend(holder.instanceOptions.themes[options.theme], null);
                                ...
                                if (options.fixedSize != null) holder.fixedSize = utils.truthy(options.fixedSize);
                        ...
                        function prepareDOMElement(prepSettings) {
                            ...
                            if (theme.text) {
                                ...
                                if (holderTemplateMatches !== null) holderTemplateMatches.forEach((function(match) {
                                    if (match === "holder_dimensions") theme.text = theme.text.replace(match, dimensionsCaption);
                                }));
                            }
                            ...
                            if (flags.font) {
                                ...
                                if (!engineSettings.noFontFallback && el.nodeName.toLowerCase() === "img" && App.setup.supportsCanvas && engineSettings.renderer === "svg") engineSettings = extend(engineSettings, {
                                    renderer: "canvas"
                                });
                            }
                            if (flags.font && engineSettings.renderer == "canvas") engineSettings.reRender = true;
                            if (mode == "background") {
                                if (el.getAttribute("data-background-src") == null) DOM.setAttr(el, {
                            ...
                            if (mode == "image" || mode == "fluid") DOM.setAttr(el, {
                            ...
                            if (mode == "image") {
                                ...
                                if (engineSettings.renderer == "html") el.style.backgroundColor = theme.bg; else {
                            ...
                            } else if (mode == "background" && engineSettings.renderer != "html") render(renderSettings); else if (mode == "fluid") {
                                el.holderData.resizeUpdate = true;
                                if (dimensions.height.slice(-1) == "%") el.style.height = dimensions.height; else if (flags.auto == null || !flags.auto) el.style.height = dimensions.height + "px";
                        ...
                        function render(renderSettings) {
                            ...
                            if (image == null) throw "Holder: couldn't render placeholder";
                            if (mode == "background") {
                                el.style.backgroundImage = "url(" + image + ")";
                                if (!engineSettings.noBackgroundSize) el.style.backgroundSize = scene.width + "px " + scene.height + "px";
                            } else {
                                if (el.nodeName.toLowerCase() === "img") DOM.setAttr(el, {
                                    src: image
                                }); else if (el.nodeName.toLowerCase() === "object") DOM.setAttr(el, {
                                    data: image,
                                    type: "image/svg+xml"
                                });
                                if (engineSettings.reRender) global.setTimeout((function() {
                        ...
                        function buildSceneGraph(scene) {
                            ...
                            if (parseFloat(scene.theme.size)) fontSize = scene.theme.size; else if (parseFloat(scene.flags.size)) fontSize = scene.flags.size;
                            scene.font = {
                            ...
                            if (!tpdata) throw "Holder: staging fallback not supported yet.";
                            ...
                            if (tpdata.lineCount > 1) {
                                ...
                                if (scene.align === "left" || scene.align === "right") maxLineWidth = scene.width * (1 - (1 - lineWrap) * 2);
                                ...
                                finalizeLine(holderTextGroup, line, offsetX, holderTextGroup.properties.leading);
                                holderTextGroup.add(line);
                                if (scene.align === "left") holderTextGroup.moveTo(scene.width - sceneMargin, null, null); else if (scene.align === "right") {
                                ...
                                if ((scene.height - holderTextGroup.height) / 2 < 0) holderTextGroup.moveTo(null, 0, null);
                            } else {
                                textNode = new Shape.Text(scene.text);
                                line = new Shape.Group("line0");
                                line.add(textNode);
                                holderTextGroup.add(line);
                                if (scene.align === "left") holderTextGroup.moveTo(scene.width - sceneMargin, null, null); else if (scene.align === "right") holderTextGroup.moveTo(0 - (scene.width - sceneMargin), null, null); else holderTextGroup.moveTo((scene.width - tpdata.boundingBox.width) / 2, null, null);
                        ...
                        function updateResizableElements(element) {
                            var images;
                            if (element == null || element.nodeType == null) images = App.vars.resizableImages; else images = [ element ];
                            for (var i = 0, l = images.length; i < l; i++) {
                                ...
                                if (el.holderData) {
                                    ...
                                    if (dimensions) {
                                        if (!el.holderData.resizeUpdate) continue;
                        ...
                        function visibilityCheck() {
                            ...
                            if (renderableImages.length) Holder.run({
                        ...
                        var stagingRenderer = function() {
                            ...
                            return function(graph) {
                                ...
                                if (App.setup.supportsSVG) {
                                    ...
                                    if (svg == null || svg.parentNode !== document.body) firstTimeSetup = true;
                        ...
                        (function() {
                            ...
                            if (canvas.getContext) if (canvas.toDataURL("image/png").indexOf("data:image/png") != -1) {
                        ...
                        if (onDomReady) onDomReady((function() {
                            if (!App.vars.preempted) Holder.run();
                            if (global.addEventListener) {
                                global.addEventListener("resize", resizeEvent, false);
                                global.addEventListener("orientationchange", resizeEvent, false);
                            } else global.attachEvent("onresize", resizeEvent);
                            if (typeof global.Turbolinks == "object") global.document.addEventListener("page:change", (function() {
                                Holder.run();
                            }));
                        }));
	 ...
	 */
                    function _onDomReady(win) {
                        ...
                        function ready(fn) {
                            if (!isReady) {
                                ...
                                isReady = true;
                                while (fn = callbacks.shift()) defer(fn);
                            }
                        }
                        ...
                        if (doc[READYSTATE] === COMPLETE) defer(ready); else if (w3c) {
                            ...
                            if (_top && _top.doScroll) (function doScrollCheck() {
                                if (!isReady) {
                                    try {
                                        _top.doScroll("left");
                                    } catch (e) {
                        ...
                        for (var i = 0; i < pairs.length; i++) {
                            ...
                            if (m = objectRegex.test(key)) {
                                ...
                                while (m.length) {
                                    ...
                                    if (!ctx[prop]) ctx[prop] = {}; else if (ctx[prop] && typeof ctx[prop] !== "object") break;
                    ...
                    exports.stringify = function(obj) {
                        ...
                        for (var key in obj) {
                            var value = obj[key];
                            if ("array" == type(value)) {
                                for (var i = 0; i < value.length; ++i) pairs.push(encode(key + "[" + i + "]") + "=" + encode(value[i]));
                                continue;
                            }
                    ...
                    var SceneGraph = function(sceneProperties) {
                        ...
                        function merge(parent, child) {
                            for (var prop in child) parent[prop] = child[prop];
                            return parent;
                        }
                        ...
                        SceneNode.prototype.add = function(child) {
                            ...
                            if (typeof this.children[name] === "undefined") {
                                ...
                                child.parent = this;
                            } else throw "SceneGraph: child already exists: " + name;
                        };
                        ...
                        var Shape = function(name, props) {
                            ...
                            if (typeof props !== "undefined") merge(this.properties, props); else if (typeof name !== "undefined" && typeof name !== "string") throw "SceneGraph: invalid node name";
                        };
                    ...
                    (function(global) {
                        exports.extend = function(a, b) {
                            var c = {};
                            for (var x in a) if (Object.prototype.hasOwnProperty.call(a, x)) c[x] = a[x];
                            if (b != null) for (var y in b) if (Object.prototype.hasOwnProperty.call(b, y)) c[y] = b[y];
                            return c;
                        };
                        exports.cssProps = function(props) {
                            var ret = [];
                            for (var p in props) if (Object.prototype.hasOwnProperty.call(props, p)) ret.push(p + ":" + props[p]);
                            return ret.join(";");
                        };
                        exports.encodeHtmlEntity = function(str) {
                            ...
                            for (var i = str.length - 1; i >= 0; i--) {
                                ...
                                if (charCode > 128) buf.unshift([ "&#", charCode, ";" ].join("")); else buf.unshift(str[i]);
                            }
                            return buf.join("");
                        };
                        ...
                        exports.dimensionCheck = function(el) {
                            ...
                            if (dimensions.height && dimensions.width) return dimensions; else return false;
                        };
                        exports.truthy = function(val) {
                            if (typeof val === "string") return val === "true" || val === "yes" || val === "1" || val === "on" || val === "✓";
                            return !!val;
                        };
                    ...
                    (function(global) {
                        var DOM = __nested_webpack_require_67337__(9);
                        ...
                        exports.svgStringToDataURI = function() {
                            ...
                            return function(svgString, base64) {
                                if (base64) return base64Prefix + btoa(global.unescape(encodeURIComponent(svgString))); else return rawPrefix + encodeURIComponent(svgString);
                            };
                        }();
                    ...
                    (function(global) {
                        exports.newEl = function(tag, namespace) {
                            ...
                            if (namespace == null) return global.document.createElement(tag); else return global.document.createElementNS(namespace, tag);
                        };
                    ...
                    var Color = function(color, options) {
                        ...
                        if (color.length === 3) color = color.replace(/./g, "$&$&");
                        if (color.length !== 6) return;
                        this.alpha = 1;
                        if (options && options.alpha) this.alpha = options.alpha;
                    ...
                    module.exports = function(sceneGraph, renderSettings) {
                        ...
                        if (outlineProperties) sceneContent.push(outline);
                        ...
                        if (/&amp;(x)?#[0-9A-Fa-f]/.test(output[0])) output = output.replace(/&amp;#/gm, "&#");
                    ...
                    function shaven(arrayOrObject) {
                        ...
                        if (!isArray && objType !== "object") throw new Error("Argument must be either an array or an object " + "and not " + JSON.stringify(arrayOrObject));
                        if (isArray && arrayOrObject.length === 0) return {};
                        var config = {};
                        var elementArray = [];
                        if (Array.isArray(arrayOrObject)) elementArray = arrayOrObject.slice(0); else {
                        ...
                        function createElement(sugarString) {
                            ...
                            if (properties.class) element.attr.class = properties.class;
                        ...
                        function buildDom(elemArray) {
                            if (Array.isArray(elemArray) && elemArray.length === 0) return {};
                            var index = 1;
                            ...
                            if (typeof array[0] === "string") array[0] = createElement(array[0]); else if (Array.isArray(array[0])) index = 0; else throw new Error("First element of array must be a string, " + "or an array and not " + JSON.stringify(array[0]));
                            for (;index < array.length; index++) if (array[index] === false || array[index] === null) {
                                array[0] = false;
                                break;
                            } else if (array[index] === void 0 || array[index] === true) continue; else if (typeof array[index] === "string") {
                                if (config.escapeHTML) array[index] = escape.HTML(array[index]);
                                array[0].children.push(array[index]);
                            } else if (typeof array[index] === "number") array[0].children.push(array[index]); else if (Array.isArray(array[index])) {
                                ...
                                if (array[index][0]) array[0].children.push(array[index][0]);
                            } else if (typeof array[index] === "function") createdCallback = array[index]; else if (_typeof(array[index]) === "object") for (var attributeKey in array[index]) {
                                if (!array[index].hasOwnProperty(attributeKey)) continue;
                                var attributeValue = array[index][attributeKey];
                                if (array[index].hasOwnProperty(attributeKey) && attributeValue !== null && attributeValue !== false) array[0].attr[attributeKey] = (0, 
                                _mapAttributeValue2.default)(attributeKey, attributeValue);
                            } else throw new TypeError('"' + array[index] + '" is not allowed as a value');
                            if (array[0] !== false) {
                                var HTMLString = "<" + array[0].tag;
                                for (var key in array[0].attr) if (array[0].attr.hasOwnProperty(key)) {
                                    var _attributeValue = escape.attribute(array[0].attr[key]);
                                    var value = _attributeValue;
                                    if (config.quoteAttributes || /[ "'=<>]/.test(_attributeValue)) value = config.quotationMark + _attributeValue + config.quotationMark;
                    ...
                    exports.default = function(sugarString) {
                        ...
                        if (classes) properties.class = classes.join(" ").replace(/\./g, "");
                        if (references) properties.reference = references[1];
                        if (sugarString.endsWith("&") || sugarString.endsWith("!")) properties.escapeHTML = false;
                        return properties;
                    };
                }, function(module, exports) {
                    "use strict";
                    ...
                    exports.default = function(key, value) {
                        ...
                        if (key === "style" && (typeof value === "undefined" ? "undefined" : _typeof(value)) === "object") return (0, 
                        _stringifyStyleObject2.default)(value);
                        if (key === "transform" && Array.isArray(value)) return (0, _buildTransformString2.default)(value);
                        return value;
                    };
                }, function(module, exports) {
                    "use strict";
                    ...
                    exports.default = function(transformObjects) {
                        return transformObjects.map((function(transformation) {
                            var values = [];
                            if (transformation.type === "rotate" && transformation.degrees) values.push(transformation.degrees);
                    ...
                    function sanitizeProperties(key, value) {
                        if (value === null || value === false || value === void 0) return;
	 ...
	 */                        function compare(a, b) {
                        ...
                        function truncate(s, n) {
                            if (typeof s === "string") return s.length < n ? s : s.slice(0, n); else return s;
                        }
                        ...
                        function _deepEqual(actual, expected, strict, memos) {
                            if (actual === expected) return true; else if (isBuffer(actual) && isBuffer(expected)) return compare(actual, expected) === 0; else if (util.isDate(actual) && util.isDate(expected)) return actual.getTime() === expected.getTime(); else if (util.isRegExp(actual) && util.isRegExp(expected)) return actual.source === expected.source && actual.global === expected.global && actual.multiline === expected.multiline && actual.lastIndex === expected.lastIndex && actual.ignoreCase === expected.ignoreCase; else if ((actual === null || typeof actual !== "object") && (expected === null || typeof expected !== "object")) return strict ? actual === expected : actual == expected; else if (isView(actual) && isView(expected) && pToString(actual) === pToString(expected) && !(actual instanceof Float32Array || actual instanceof Float64Array)) return compare(new Uint8Array(actual.buffer), new Uint8Array(expected.buffer)) === 0; else if (isBuffer(actual) !== isBuffer(expected)) return false; else {
                                memos = memos || {
                                    actual: [],
                                    expected: []
                                };
                                var actualIndex = memos.actual.indexOf(actual);
                                if (actualIndex !== -1) if (actualIndex === memos.expected.indexOf(expected)) return true;
	...
	*/
                    ...
                    (function(global, process) {
                        ...
                        return this;
                    }(), __nested_webpack_require_112702__(24));
                }, function(module, exports) {
            ...
            (function(ctx, isMeteorPackage) {
                if (isMeteorPackage) Holder = ctx.Holder;
        ...
        document.querySelectorAll(".talk-about-us [data-tippy]").forEach((item => {
            ...
            if (bullet && content) {
                ...
                tippy_esm(bullet, {
                    ...
                    trigger: "focus click",
                    onTrigger: e => {
                        e.reference.setAttribute("aria-label", "Приховати відгук");
                    },
                    onUntrigger: e => {
                        e.reference.setAttribute("aria-label", "Показати відгук");
                    }
                });
            }
        }));
...
*/
        ...
        window.addEventListener("load", (function(e) {
            ...
            function initCompanyMap() {
                companyMapItems.forEach(((companyMapItem, index) => {
                    ...
                    if (index !== companyMapItems.length - 1) {
                        ...
                        let start = null;
                        if (prevItem && prevSolidLine) start = prevSolidLine.getBoundingClientRect().top + prevSolidLine.getBoundingClientRect().height - companyMapItem.getBoundingClientRect().top + "px";
            ...
            function uninitCompanyMap() {
                companyMapItems.forEach(((companyMapItem, index) => {
                    ...
                    if (index !== companyMapItems.length - 1) gsapWithCSS.killTweensOf(dot); else gsapWithCSS.killTweensOf(companyMapItem);
            ...
            function drawLine(line, progress) {
                line.setAttribute("stroke-dashoffset", line.getTotalLength() - line.getTotalLength() * progress);
            }
        ...
        function setHeaderTop() {
            document.documentElement.style.setProperty("--header-top", document.querySelector("header").getBoundingClientRect().top * -1 + "px");
        }
        window.addEventListener("load", setHeaderTop);
        function setVH() {
            let vh = (getWindowHeight() * .01).toFixed(2);
        ...
        function burger() {
            ...
            burger.addEventListener("click", (() => {
                if (document.documentElement.classList.contains("_menu-open")) closeMenu(); else {
                    openMenu();
                    setHeaderTop();
                }
            }));
            function closeMenu() {
                ...
                document.querySelector(".burger").setAttribute("aria-label", "Відкрити меню");
            }
            function openMenu() {
                document.documentElement.classList.add("_menu-open");
                document.documentElement.classList.add("_lock");
                document.querySelector(".burger").setAttribute("aria-label", "Закрити меню");
            }
        ...
        function updateSmallMobileScreen(e) {
            if (e.matches) headerInfoMobile.insertAdjacentElement("beforeend", headerItemTel); else headerInfo.insertAdjacentElement("beforeend", headerItemTel);
        }
        ...
        const resizeHeaderObserver = new ResizeObserver((entries => {
            for (let i = 0; i < entries.length; i++) {
                ...
                if (typeof height === "number" && height !== prevHeaderHeight.get(entries[i].target)) {
                    ...
                    setHeaderTop();
                    if (header.classList.contains("_fixed")) document.documentElement.style.setProperty("--header-top-height", headerTop.offsetHeight + "px");
                    break;
                }
            }
        }));
            ...
            whyItems.forEach((whyItem => {
                ...
                function onClick() {
                    ...
                    if (whyItem.classList.contains("_active")) {
                        ...
                        whyItem.querySelector(".why-item__icon").setAttribute("aria-label", "Показати детальну інформацію");
                    } else {
                        ...
                        if (activeWhyItem) {
                            ...
                            activeWhyItem.querySelector(".why-item__icon").setAttribute("aria-label", "Показати детальну інформацію");
                        }
                        slideDown(whyItemText);
                        whyItem.classList.add("_active");
                        whyItem.querySelector(".why-item__icon").setAttribute("aria-label", "Приховати детальну інформацію");
                    }
                    setWhyItemWidth(whyItem, icon);
                }
            }));
        ...
        function updateMobileScreen(e) {
            if (e.matches) {
                whyItems.forEach((item => {
                    item.querySelector(".why-item__icon").setAttribute("tabindex", "-1");
                    item.querySelector(".why-item__icon").setAttribute("aria-hidden", "true");
                    item.querySelector(".why-item__text").hidden = null;
                    if (item.classList.contains("_active")) {
                        item.classList.remove("_active");
                        item.querySelector(".why-item__icon").setAttribute("aria-label", "Показати детальну інформацію");
                    }
                }));
                ...
                whyItems.forEach((item => {
                    ...
                    item.querySelector(".why-item__icon").removeAttribute("tabindex");
                    item.querySelector(".why-item__icon").removeAttribute("aria-hidden");
                }));
        ...
        function setWhyItemWidth(whyItem, icon = null) {
            ...
            if (whyItem.classList.contains("why-item_right")) whyItem.style.width = whyInnerRect.width + whyInnerRect.left - iconRect.left + "px"; else whyItem.style.width = iconRect.width + iconRect.left - whyInnerRect.left + "px";
        }
        document.querySelectorAll("[data-tabs]").forEach((tabBlock => {
            let tabTitles = tabBlock.querySelectorAll("[data-tabs-nav] > *");
            let tabBodyItems = tabBlock.querySelectorAll("[data-tabs-content] > *");
            let activeTab = tabBlock.querySelector("[data-tabs-nav] > ._tab-active");
            tabBodyItems.forEach((tabBody => {
                tabBody.style.display = "none";
            }));
            if (activeTab) activeTab.style.removeProperty("display"); else tabBodyItems[0].style.removeProperty("display");
            tabTitles.forEach(((tabTitle, index) => {
                const tabBody = tabBodyItems[index];
        ...
        awardsTabBlocks.forEach((awardsTabs => {
            ...
            const tabBodyItems = awardsTabs.querySelectorAll("[data-tabs-content] > *");
            let showTabsCount = 3;
            ...
            if (isMobile.any()) {
                awardsNavWrapper.addEventListener("touchstart", onAwardsNavTouchStart, {
                    passive: true
                });
                awardsNavWrapper.addEventListener("touchend", onAwardsNavTouchEnd);
                awardsContent.addEventListener("touchstart", onAwardsContentTouchStart, {
                    passive: true
                });
                awardsContent.addEventListener("touchend", onAwardsContentTouchEnd);
            } else {
                awardsNavWrapper.addEventListener("mousedown", onAwardsNavTouchStart);
                awardsNavWrapper.addEventListener("mouseup", onAwardsNavTouchEnd);
                awardsContent.addEventListener("mousedown", onAwardsContentTouchStart);
                awardsContent.addEventListener("mouseup", onAwardsContentTouchEnd);
            }
            awardsNavWrapper.addEventListener("click", onTabClick);
            function onAwardsNavTouchStart(e) {
                setStartPoints(e);
                const moveEvent = e.type === "touchstart" ? "touchmove" : "mousemove";
                if (getWindowWidth() >= 768) awardsNavWrapper.addEventListener(moveEvent, onAwardsNavVerticalMove, {
                    passive: false
                }); else awardsNavWrapper.addEventListener(moveEvent, onAwardsNavHorisontalMove, {
                    passive: true
                });
            }
            function onAwardsNavTouchEnd(e) {
                resetVariables();
                const moveEvent = e.type === "touchend" ? "touchmove" : "mousemove";
                if (getWindowWidth() >= 768) awardsNavWrapper.removeEventListener(moveEvent, onAwardsNavVerticalMove); else awardsNavWrapper.removeEventListener(moveEvent, onAwardsNavHorisontalMove);
            }
            function onAwardsNavHorisontalMove(e) {
                handleMove(e, "horizontal");
            }
            function onAwardsNavVerticalMove(e) {
                handleMove(e, "vertical");
            }
            function onTabClick(e) {
                if (moved) return;
                let activeTab = awardsTabs.querySelector("._tab-active");
                if (e.target !== activeTab) changeActiveTabBody(activeTab, e.target);
                disabledButton();
            }
            function onAwardsContentTouchStart(e) {
                setStartPoints(e);
                const moveEvent = e.type === "touchstart" ? "touchmove" : "mousemove";
                awardsContent.addEventListener(moveEvent, onAwardsContentMove, {
                    passive: true
                });
            }
            function onAwardsContentTouchEnd(e) {
                resetVariables();
                const moveEvent = e.type === "touchend" ? "touchmove" : "mousemove";
                awardsContent.removeEventListener(moveEvent, onAwardsContentMove);
            }
            function onAwardsContentMove(e) {
            ...
            function handleMove(e, direction = null, isSetNewVisibleTabs = true) {
                if (moved) return;
                let xEnd = null;
                let yEnd = null;
                if (e.type === "touchmove") {
                ...
                function verticalSwipe() {
                    if (yDiff > 0 && yDiff > sensitivity) {
                        onNext(isSetNewVisibleTabs);
                        moved = true;
                    } else if (yDiff < 0 && yDiff < -sensitivity) {
                        onPrev(isSetNewVisibleTabs);
                        moved = true;
                    }
                }
                ...
                function bothSwipe() {
                    if (Math.abs(xDiff) > Math.abs(yDiff)) horizontalSwipe(); else if (Math.abs(xDiff) < Math.abs(yDiff)) verticalSwipe();
                }
            }
            const breakpoints = {
                0: 1,
            ...
            for (let i = 0; i < breakpointsArray.length; i++) {
                ...
                const matchMedia = window.matchMedia(`(min-width: ${currentMedia}px) ${nextMedia ? "and (max-width: " + (nextMedia - .02) + "px)" : ""}`);
                matchMedia.addEventListener("change", (e => onBreakpoint(e, currentVisibleTabsCount)));
                onBreakpoint(matchMedia, currentVisibleTabsCount);
            }
            function onBreakpoint(e, showVisibleTabCount) {
                if (!e.matches) return;
                let activeTab = awardsTabs.querySelector("._tab-active");
                let activeTabIndex = +activeTab.getAttribute("data-index");
                showTabsCount = showVisibleTabCount;
                if (activeTabIndex > tabTitles.length - showTabsCount) visibleTabIndex = {
                    first: tabTitles.length - showTabsCount > 0 ? tabTitles.length - showTabsCount : 0,
                    last: tabTitles.length - 1
                }; else visibleTabIndex = {
            ...
            let changeNavHeightTimeoutId = null;
            const navigationRO = new ResizeObserver((entries => {
                clearTimeout(changeNavHeightTimeoutId);
                changeNavHeightTimeoutId = setTimeout((() => {
                    changeNavHeight();
                }), 100);
            }));
            ...
            function changeVisibleTabs() {
                ...
                for (let i = visibleTabIndex.first; i <= visibleTabIndex.last; i++) tabTitles[i].classList.add("_tab-visible");
                disabledButton();
            }
            function changeNavHeight() {
                if (getWindowWidth() >= 768) {
                    let wrapperHeight = tabTitles[visibleTabIndex.last].getBoundingClientRect().top + tabTitles[visibleTabIndex.last].getBoundingClientRect().height - tabTitles[visibleTabIndex.first].getBoundingClientRect().top;
                    let transformY = tabTitles[visibleTabIndex.first].getBoundingClientRect().top - navigation.getBoundingClientRect().top;
                    ...
                    let transformX = (tabTitles[visibleTabIndex.first].getBoundingClientRect().left - navigation.getBoundingClientRect().left) * 100 / navigation.getBoundingClientRect().width;
                    navigationWrapper.style.height = wrapperHeight + "px";
                    navigation.style.transform = `translate3d(${-transformX}%,0,0)`;
                }
            }
            function onNext(isSetNewVisibleTabs = false) {
                if (isSetNewVisibleTabs && visibleTabIndex.last === tabTitles.length - 1) return;
                let activeTab = awardsTabs.querySelector("._tab-active");
                let activeTabIndex = +activeTab.getAttribute("data-index");
                if (activeTabIndex === tabTitles.length - 1) return;
                let nextTab, nextTabIndex;
                if (isSetNewVisibleTabs) nextTabIndex = visibleTabIndex.last < tabTitles.length - showTabsCount ? visibleTabIndex.last + 1 : tabTitles.length - showTabsCount; else nextTabIndex = activeTabIndex + 1;
                nextTab = tabTitles[nextTabIndex];
                changeActiveTabBody(activeTab, nextTab);
                disabledButton();
                if (!isSetNewVisibleTabs && nextTabIndex <= visibleTabIndex.last) return;
            ...
            function onPrev(isSetNewVisibleTabs = false) {
                if (isSetNewVisibleTabs && visibleTabIndex.first === 0) return;
                let activeTab = awardsTabs.querySelector("._tab-active");
                let activeTabIndex = +activeTab.getAttribute("data-index");
                if (activeTabIndex === 0) return;
                let prevTab, prevTabIndex;
                if (isSetNewVisibleTabs) prevTabIndex = visibleTabIndex.first - 1 < showTabsCount ? showTabsCount - 1 : visibleTabIndex.first - 1; else prevTabIndex = activeTabIndex - 1;
                prevTab = tabTitles[prevTabIndex];
                changeActiveTabBody(activeTab, prevTab);
                disabledButton();
                if (!isSetNewVisibleTabs && prevTabIndex >= visibleTabIndex.first) return;
            ...
            function changeActiveTabBody(currentTab, clickedTab) {
                let currentTabIndex = +currentTab.getAttribute("data-index");
                let clickedTabIndex = +clickedTab.getAttribute("data-index");
                tabBodyItems[currentTabIndex].style.display = "none";
                currentTab.classList.remove("_tab-active");
                clickedTab.classList.add("_tab-active");
                tabBodyItems[clickedTabIndex].style.removeProperty("display");
            }
            function disabledButton() {
                let activeTab = awardsTabs.querySelector("._tab-active");
                let activeTabIndex = +activeTab.getAttribute("data-index");
                if (activeTabIndex === tabTitles.length - 1) buttonNext.disabled = true; else if (buttonNext.disabled) buttonNext.disabled = false;
                if (activeTabIndex === 0) buttonPrev.disabled = true; else if (buttonPrev.disabled) buttonPrev.disabled = false;
            }
        }));
